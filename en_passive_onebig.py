...
def allpass(septets):
    ...
    fin = 0
    # the attempt to separate root and non-root occurences of passives didnot work out in contrastive perspective
    # because of different annotation strategies wrt modal verbs - English tags them as aux, while Russian as root
    for w in septets:
        if w[3] != 'root' and 'VerbForm=Part|Voice=Pass' in w[6]: #amod, nmod или в придаточных
            ispass_nonroot = True
            id = w[0]
            for w1 in septets:
        ...
        if w[3] == 'root' and 'VerbForm=Part|Voice=Pass' in w[6]:
            ispass = True
            id = w[0]
            for w1 in septets:
                if w1[1] == id and w1[3] != 'aux:pass':  # nsubj
        ...
        if 'VerbForm=Fin|Voice=Pass' in w[6]:
            ...
            for w1 in septets:
                if w1[1] == id and w1[3] == 'nsubj:pass':
                    all_passsubj_fin += 1
        if 'VerbForm=Part|Voice=Pass' in w[6]:
            id = w[0]
            for w1 in septets:
                if w1[1] == id and w1[3] == 'nsubj:pass':
                    all_passsubj_spp += 1
...
inside = OrderedDict([('rootSPP_anal', []), ('nonrootSPP_anal', []),  ('rootSPP_morph', []), ('nonrootSPP_morph', []),
                                                                 ('passFin', []), ('NO_nsubjpass', []), ('nsubjpass', []),
])

deps = {} # dict of types of passive verb dependants, except aux regardless root or non roots
# nsubj = {} # dict for pos types of nsubjpass

print('corpus\troot+SPP+aux\t-root+SPP+aux\troot+SPP-aux\t-root+SPP-aux\tFinite\tNO-nsubjpass\tYES-nsubj')
...
# print(count_analitpass_nonroot, '\t', count_morphpass_nonroot, '\t', nodeps)
    ...
    inside['NO_nsubjpass'].append(six_pass_types[5])
    inside['nsubjpass'].append(six_pass_types[6])
...
print(doc, '\t', sum(inside['rootSPP_anal']), '\t',
  ...
  '\t', sum(inside['passFin']), '\t', sum(inside['NO_nsubjpass']), '\t', sum(inside['nsubjpass']))

print('passive verbs extends the following types of dependences in ', doc, ':','\t', deps, file=sys.stderr)
# print('counts and PoS tags for all nsubj:pass dependents regardless of pass verb type: ','\t',nsubj, file=sys.stderr)